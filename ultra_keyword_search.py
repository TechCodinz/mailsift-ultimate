            ...
            if search_type == 'exact':
                ...
                if ML_AVAILABLE
                    ...
                    similarity = cosine_similarity(
                        [query_vector], [email_vector])[0][0]
                    return float(similarity)
                else:
                    # Fallback to fuzzy matching
                    return self._calculate_relevance_score(
                        email, query_keywords, 'fuzzy')

            else:  # 'all'
                # Combine all methods
                exact_score = self._calculate_relevance_score(
                    email, query_keywords, 'exact')
                fuzzy_score = self._calculate_relevance_score(
                    email, query_keywords, 'fuzzy')
                semantic_score = self._calculate_relevance_score(
                    email, query_keywords, 'semantic')
    ...
    def search_emails(self, query: SearchQuery) -> List[SearchResult]:
        ...
        try:
            ...
            if cache_key in self.search_cache:
                ...
                if time.time() - \
                        cached_result['timestamp'] < 300:  # 5 minute cache
            ...
            for email in candidate_emails:
                relevance_score = self._calculate_relevance_score(
                    email, query.keywords, query.search_type)
