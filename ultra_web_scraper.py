...
class UltraWebScraper:
    ...
    def _check_robots_txt(self, url: str) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                # Simple robots.txt check - in production, use proper robots.txt parser
    ...
    def _handle_cloudflare(self, response: requests.Response) -> Optional[requests.Response]:
        ...
        if 'cloudflare' in response.headers.get('server', '').lower():
            ...
            try:
                return self.session.get(response.url, timeout=self.config.timeout)
            except Exception as e:
        ...
        try:
            ...
            for link in soup.find_all('a', href=True):
                ...
                if absolute_url.startswith(('http://', 'https://')):
                    ...
                    if parsed.netloc == urlparse(base_url).netloc:  # Same domain only
    ...
    def _detect_contact_pages(self, html: str, url: str) -> bool:
        ...
        if 'contact' in html_lower and ('form' in html_lower or 'input' in html_lower):
            contact_score += 5
        
        # Check for email patterns in content
        email_patterns = len(re.findall(r'\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b', html))
    ...
    def scrape_single_url(self, url: str) -> ScrapingResult:
        ...
        try:
            ...
            if 'html' in content_type:
                extraction_result = ultra_extractor.extract_emails_ultra(response.text, "html")
            else:
                extraction_result = ultra_extractor.extract_emails_ultra(response.text, "text")
            
            # Extract additional metadata
            metadata = {
                'content_length': len(response.text),
                'is_contact_page': self._detect_contact_pages(response.text, url),
                'links_found': len(self._extract_links(response.text, url)),
    ...
    def discover_contact_pages(self, base_url: str, max_pages: int = 10) -> List[str]:
            ...
            while to_visit and len(contact_pages) < max_pages:
                ...
                if result.success and result.metadata:
                    ...
                    if result.metadata.get('links_found', 0) > 0:
                        ...
                        try:
                            soup = BeautifulSoup(result.emails, 'html.parser')  # This is wrong, we need the HTML
                            # We need to store HTML in the result for this to work
