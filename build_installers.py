...
def create_desktop_app():
    ...
    try:
        subprocess.run(["python"
            "-m", "PyInstaller", "--version"], check=True, capture_output=True)
...
def create_windows_installer():
    ...
    try:
        ...
        print("❌ NSIS
            found. Please install NSIS to create Windows installer.")
...
section "install"
    ...
    createShortCut "$SMPROGRAMS\\${APPNAME}\\${APPNAME}.lnk" "$INSTDIR\\MailSif
        t_Ultra.exe" "" "$INSTDIR\\mail_icon.ico"
    createShortCut "$DESKTOP\\${APPNAME}.lnk" "$INSTDIR\\MailSift_Ultra.exe" ""
         "$INSTDIR\\mail_icon.ico"

    WriteRegStr HKLM "Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\
        ${APPNAME}" "DisplayName" "${APPNAME}"
    WriteRegStr HKLM "Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\
        ${APPNAME}" "UninstallString" "$\\"$INSTDIR\\uninstall.exe$\\""
    WriteRegStr HKLM "Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\
        ${APPNAME}" "QuietUninstallString" "$\\"$INSTDIR\\uninstall.exe$\\" /S"
    WriteRegStr HKLM "Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\
        ${APPNAME}" "InstallLocation" "$\\"$INSTDIR$\\""
    WriteRegStr HKLM "Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\
        ${APPNAME}" "DisplayIcon" "$\\"$INSTDIR\\mail_icon.ico$\\""
    WriteRegStr HKLM "Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\
        ${APPNAME}" "Publisher" "${COMPANYNAME}"
    WriteRegStr HKLM "Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\
        ${APPNAME}" "HelpLink" "${HELPURL}"
    WriteRegStr HKLM "Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\
        ${APPNAME}" "URLUpdateInfo" "${UPDATEURL}"
    WriteRegStr HKLM "Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\
        ${APPNAME}" "URLInfoAbout" "${ABOUTURL}"
    WriteRegStr HKLM "Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\
        ${APPNAME}" "DisplayVersion" "${VERSIONMAJOR}.${VERSIONMINOR}.${VERSIONBUILD}"
    WriteRegDWORD HKLM "Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall
        \\${APPNAME}" "VersionMajor" ${VERSIONMAJOR}
    WriteRegDWORD HKLM "Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall
        \\${APPNAME}" "VersionMinor" ${VERSIONMINOR}
    WriteRegDWORD HKLM "Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall
        \\${APPNAME}" "NoModify" 1
    WriteRegDWORD HKLM "Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall
        \\${APPNAME}" "NoRepair" 1
    WriteRegDWORD HKLM "Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall
        \\${APPNAME}" "EstimatedSize" ${INSTALLSIZE}
sectionEnd
...
"""
    ...
    # Build installer
    if not run_command("makensis MailSift_Ultra_Installer.nsi"
...
def create_macos_installer():
    ...
    # Copy executable
    shutil.copy("dist/MailSift_Ultra"
        app_path / "Contents" / "MacOS" / "MailSift_Ultra")
...
<dict>
    ...
    if Path("mail_icon.ico").exists():
        ...
        shutil.copy("mail_icon.ico"
            app_path / "Contents" / "Resources" / "mail_icon.ico")
...
def create_linux_installer():
    ...
    (appdir
        "usr" / "share" / "icons" / "hicolor" / "256x256" / "apps").mkdir(parents=True)

    # Copy executable
    shutil.copy("dist/MailSift_Ultra"
        appdir / "usr" / "bin" / "mailsift-ultra")
...
"""
    ...
    with open(appdir
        "usr" / "share" / "applications" / "mailsift-ultra.desktop", "w") as f:
...
<component type="desktop-application">
  ...
  <description>
    <p>MailSift Ultra is a professional email extraction
        validation tool powered by artificial intelligence.</p>
  </description>
  ...
  <url type="help">https://mailsift.com/docs</url>
    ...
    with open(appdir
        "usr" / "share" / "metainfo" / "com.mailsift.ultra.appdata.xml", "w") as f:
        f.write(appstream)

    # Copy icon
    if Path("mail_icon.ico").exists():
        shutil.copy("mail_icon.ico", appdir / "usr"
            "share" / "icons" / "hicolor" / "256x256" / "apps" / "mailsift-ultra.ico")
...
def create_icon():
    ...
    try:
        ...
        # Save as ICO
        img.save("mail_icon.ico"
            format="ICO", sizes=[(64, 64), (32, 32), (16, 16)])
        print("✅ Created proper icon file")

    except ImportError:
        print("📝 PIL not available, creating placeholder icon")
        # Create a minimal ICO file header
        ico_data = b'\x00\x00\x01\x00\x01\x00\x10\x10\x00\x00\x01\x00\x20\x00\x
            68\x04\x00\x00\x16\x00\x00\x00'
