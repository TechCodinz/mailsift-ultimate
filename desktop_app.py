...
class MailSiftDesktop:
    def __init__(self):
        ...
        self.style.configure('TLabel', background='#0a0a0a', foreground='#00ffff')
        self.style.configure('TButton', background='#1a1a1a', foreground='#00ffff')
    ...
    def create_widgets(self):
        ...
        title_label = ttk.Label(header_frame, text="🚀 MailSift Pro - Professional Email Extractor", 
                               font=('Arial', 16, 'bold'))
        title_label.pack()
        
        subtitle_label = ttk.Label(header_frame, text="Extract, Validate & Export Emails with AI Intelligence", 
                                  font=('Arial', 10))
        subtitle_label.pack()
        
        # License section
        license_frame = ttk.LabelFrame(main_frame, text="🔑 License Key", padding=10)
        license_frame.pack(fill=tk.X, pady=(0, 10))
        
        license_entry = ttk.Entry(license_frame, textvariable=self.license_key, width=50)
        license_entry.pack(side=tk.LEFT, padx=(0, 10))
        
        verify_btn = ttk.Button(license_frame, text="Verify License", command=self.verify_license)
        verify_btn.pack(side=tk.LEFT, padx=(0, 10))
        
        buy_btn = ttk.Button(license_frame, text="Buy License", command=self.open_buy_page)
        buy_btn.pack(side=tk.LEFT)
        
        # Input section
        input_frame = ttk.LabelFrame(main_frame, text="📝 Input Text or Files", padding=10)
        input_frame.pack(fill=tk.BOTH, expand=True, pady=(0, 10))
        
        # Text input
        self.text_input = scrolledtext.ScrolledText(input_frame, height=8, bg='#1a1a1a', fg='#ffffff',
        ...
        ttk.Button(file_frame, text="📁 Open File", command=self.open_file).pack(side=tk.LEFT, padx=(0, 10))
        ttk.Button(file_frame, text="🌐 Extract from URL", command=self.extract_from_url).pack(side=tk.LEFT, padx=(0, 10))
        ttk.Button(file_frame, text="🧹 Clear", command=self.clear_input).pack(side=tk.LEFT)
        ...
        # Results tree
        self.results_tree = ttk.Treeview(results_frame, columns=('Status', 'Provider', 'Type'), show='tree headings')
        ...
        ttk.Button(export_frame, text="📊 Export Excel", command=self.export_excel).pack(side=tk.LEFT, padx=(0, 10))
        ttk.Button(export_frame, text="📄 Export JSON", command=self.export_json).pack(side=tk.LEFT, padx=(0, 10))
        
        # Status bar
        self.status_var = tk.StringVar()
        self.status_var.set("Ready - Enter text or open a file to extract emails")
    ...
    def check_license(self):
        ...
        try:
            config_file = os.path.join(os.path.expanduser('~'), '.mailsift_config.json')
    ...
    def verify_license(self):
        ...
        def verify():
            try:
                ...
                response = requests.post('https://api.mailsift.com/verify-license', 
                ...
                if response.status_code == 200:
                    ...
                    if data.get('valid'):
                        ...
                        self.status_var.set("License verified - Full features unlocked")
                    else:
                ...
                messagebox.showerror("Error", f"License verification failed: {str(e)}")
                self.status_var.set("License verification failed")
            finally:
                self.progress.stop()
    ...
    def save_license(self, key):
        ...
        try:
            config_file = os.path.join(os.path.expanduser('~'), '.mailsift_config.json')
            config = {'license_key': key, 'last_verified': datetime.now().isoformat()}
    ...
    def extract_from_url(self):
        ...
        url = tk.simpledialog.askstring("URL", "Enter URL to extract emails from:")
        if url:
    ...
    def update_validation_results(self):
        ...
        self.status_var.set(f"Validation complete: {valid_count}/{total_count} emails valid")
            ...
            try:
                with open(file_path, 'w', newline='', encoding='utf-8') as f:
                    ...
                    for email in self.extracted_emails:
                        provider = email.split('@')[1] if '@' in email else 'Unknown'
                        email_type = self.classify_email_type(email)
                        status = self.validation_results.get(email, {}).get('status', 'Extracted')
                        
                        writer.writerow([email, status, provider, email_type])
                
                messagebox.showinfo("Success", f"Results exported to {file_path}")
                self.status_var.set(f"Exported {len(self.extracted_emails)} emails to CSV")
            except Exception as e:
                messagebox.showerror("Error", f"Export failed: {str(e)}")
    
    def export_excel(self):
        """Export results to Excel"""
        messagebox.showinfo("Info", "Excel export requires additional setup. Please use CSV export for now.")
    ...
    def export_json(self):
        ...
        if file_path:
            try:
                ...
                messagebox.showinfo("Success", f"Results exported to {file_path}")
                self.status_var.set(f"Exported {len(self.extracted_emails)} emails to JSON")
            except Exception as e:
                messagebox.showerror("Error", f"Export failed: {str(e)}")
